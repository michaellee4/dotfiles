...
before packages are loaded."
  ...
  (with-eval-after-load 'evil
    ...
    (defun ml/regular-buffer-exists-p ()
      "Return non-nil when any normal user-facing buffer exists."
      (let ((buffers (buffer-list))
            found)
        (while (and buffers (not found))
          (setq found (ml/regular-buffer-p (car buffers)))
          (setq buffers (cdr buffers)))
        found))

    (evil-define-command ml/evil-close-current-buffer (&optional bang)
      "Close the current buffer without closing Emacs."
      :repeat nil
      (interactive "<!>")
      (when bang
        (set-buffer-modified-p nil))
      (when (kill-buffer (current-buffer))
        (unless (ml/regular-buffer-exists-p)
          (switch-to-buffer (get-buffer-create "*scratch*")))))

    (evil-define-command ml/evil-write-and-close-current-buffer (file &optional bang)
      "Save the current buffer, then close it without closing Emacs."
      :repeat nil
      (interactive "<f><!>")
      (evil-write nil nil nil file bang)
      (ml/evil-close-current-buffer bang))

    (evil-ex-define-cmd "q" 'ml/evil-close-current-buffer)
    (evil-ex-define-cmd "Q" 'ml/evil-close-current-buffer)
    (evil-ex-define-cmd "W" 'evil-write)
    (dolist (cmd '("wq" "Wq" "wQ" "WQ" "qw" "Qw" "qW" "QW"))
      (evil-ex-define-cmd cmd 'ml/evil-write-and-close-current-buffer)))
  )
  ...
  (custom-set-variables
   ...
   '(package-selected-packages
     '(a ace-link aggressive-indent all-the-icons auto-compile auto-highlight-symbol
         ...
         closql code-cells code-review column-enforce-mode company cython-mode
         deferred define-word devdocs diminish dired-quick-sort disable-mouse
         dotenv-mode drag-stuff dumb-jump elisp-def elisp-demos elisp-slime-nav
         ...
         forge ghub git-link git-messenger git-modes git-timemachine
